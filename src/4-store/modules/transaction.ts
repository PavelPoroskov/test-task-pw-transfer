...
// Reducer
export default function reducer(state: TransactionState = initState, action: ActionP ): TransactionState {
  switch (action.type) {
    case NEW:
      return {
        ...initState,
...
// Side Effects
export const commitTransactionEpic: AppEpic = (action$, state$, {client}) => action$.pipe(
  ofType(COMMIT),
  switchMap(({payload}) =>
